...
class TransformerLayer2(nn.Module):
    ...
    def forward(self, input, q_input, pad_mask=None, att_mask=None):
        emb, att = self.attention(input, input, pad_mask, att_mask)
        emb, att = self.attention3(q_input, emb, emb, pad_mask, att_mask)
        emb = self.fwd_layer(emb)
    ...
    def forward(self, source_embed, token_index=None, source_pad_mask=None, target_pad_mask=None, max_len=300, top_k=1,
        ...
        if token_index != None:  # --- Training/Testing Phase ---
            ...
            for i in range(len(self.transform2)):
                final_embed2 = self.transform2[i](final_embed, source_embed, pad_mask, att_mask = None)[0]
                # ([8, 1114, 256])
            final_embed = torch.cat([final_embed2, target_embed], dim=1)
            for i in range(len(self.transform)):
                final_embed = self.transform[i](final_embed, pad_mask, att_mask)[0]
            # Make prediction for next tokens
            token_index = torch.arange(self.num_tokens).unsqueeze(0).repeat(token_index.shape[0], 1).to(token_index.device)  # (1,K) --> (B,K) ([8, 1000])
...
class ClsGen(nn.Module):
    ...
    def forward(self, image, history=None, caption=None, label=None, threshold=0.15, bos_id=1, eos_id=2, pad_id=3,
        ...
        img_mlc, df_emb, ds_emb = self.classifier(img=image, txt=history, lbl=label, threshold=threshold, pad_id=pad_id, get_embed=True)  # (B,T,C), (B,T,E) (8,114,2),(8,114,256)
